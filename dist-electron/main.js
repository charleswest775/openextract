...
const device_watcher_1 = require("./device-watcher");
let mainWindow = null;
let sidecar = null;
 ...
 */
function handleSidecarNotification(notification) {
...
app.whenReady().then(async () => {
    ...
    const iconDir = isDev
        ...
        : path.join(process.resourcesPath || __dirname, 'assets');
    ...
    trayManager.onQuickAnalyze = (objective) => {
        // Placeholder: send analysis request to sidecar
        if (sidecar) {
            sidecar.call('analyze.quick', { objective }).catch((err) => {
    ...
    trayManager.onExtractFromBackup = async () => {
        ...
        try {
            ...
            // Run extraction
            const extractResult = await sidecar.call('extraction.start', { udid, backup_dir: backupDir }, 7200000);
    ...
    deviceWatcher = new device_watcher_1.DeviceWatcher(sidecar);
