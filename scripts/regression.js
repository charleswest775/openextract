 ...
 *   npm run regression
 *
 * Idempotent. Works on macOS, Linux, Windows (PowerShell or cmd). Steps:
 *   1. Resolve a Python 3 interpreter (py -3 on Windows, python3 on POSIX).
 *   2. Create .venv, clone the ios-backup-core sibling, install sidecar deps.
 *   3. Download Playwright's Chromium bundle (optional — Electron tests
 *      don't need it).
 *   4. Regenerate the synthetic-backup fixtures.
 *   5. Build the Electron main + renderer bundles.
 *   6. Run the Playwright suite (wraps with xvfb-run on headless Linux).
 */
...
// Run from python/ so the editable path `-e ../../ios-backup-core` in
// requirements.txt resolves to the sibling checkout above.
run(PIP, ['install', '-r', 'requirements.txt'], { cwd: path.join(REPO, 'python') });

// 3. Playwright Chromium bundle. Only needed for non-Electron browser tests;
//    `_electron.launch` uses the Electron from node_modules. Best-effort so
//    firewalled networks that block playwright.dev can still run our specs.
